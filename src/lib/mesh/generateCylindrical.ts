...
export function generateCylindrical(
  ...
  // cols+1 for seam duplication
  const seamCols = cols + 1;
  const outerCount = rows * seamCols;
  const innerCount = rows * seamCols;
  const capVerts = seamCols * 2 * 2; // top and bottom caps, inner+outer
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c <= cols; c++) {
      ...
      const rad = baseRadius + minThickness + h * thicknessRange;
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c <= cols; c++) {
      const angle = (c / cols) * Math.PI * 2;
      const y = (rows - 1 - r) * cellH - heightMM / 2;
      const rad = baseRadius - baseThickness;
  ...
  // Inner face indices (reversed winding)
  for (let r = 0; r < rows - 1; r++) {
    for (let c = 0; c < seamCols - 1; c++) {
  ...
  // Top cap (r=0) - connects outer to inner
  const topOuterStart = vi;
  for (let c = 0; c <= cols; c++) {
    const oi = c; // outer row 0
